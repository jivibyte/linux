...
get_current_time_mst_tz()
{
	...
	time_now=read TZ='America/Denver' date +%T

	# Another way of executing the piped command in bash is by using command keyword
	# time_now=command TZ='America/Denver' date +%T

	# return value of the function is current time in MST
	return $time_now
}
